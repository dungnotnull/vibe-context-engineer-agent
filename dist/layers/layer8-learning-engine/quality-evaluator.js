export class QualityEvaluator {
    evaluate(contextId, memory, feedback) {
        const dimensions = {
        ...
        const overallScore = dimensions.relevance * 0.30 +
    ...
    scoreRelevance(memory) {
        ...
        const relevantFacts = memory.facts.filter((f) => f.tags.some((t) => relevantTags.includes(t)));
    ...
    scoreAccuracy(memory, feedback) {
        if (!feedback || feedback.length === 0)
            return 0.75;
        const relevantFeedback = feedback.filter((f) => f.type === 'relevance' || f.type === 'completeness');
        if (relevantFeedback.length === 0)
            return 0.75;
        const avgFeedback = relevantFeedback.reduce((s, f) => s + f.score, 0) / relevantFeedback.length;
        return Math.min(1, avgFeedback);
    }
    scoreFreshness(memory) {
        if (memory.facts.length === 0)
            return 0;
        const now = Date.now();
    ...
    generateSuggestions(dimensions, memory) {
        const suggestions = [];
