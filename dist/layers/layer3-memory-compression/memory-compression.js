...
export class MemoryCompressionEngine {
    ...
    compress(changes) {
        ...
        for (const change of changes)
            allFacts.push(...this.extractFacts(change));
            ...
            switch (cat) {
                ...
                case 'dependency':
                    for (const dep of change.affectedDependencies)
                        facts.push({ id: randomUUID(), type: 'fact', content: `Dependency changed: ${dep}`, confidence: 0.95, timestamp: ts, source, tags: ['dependency', dep], accessCount: 0, lastAccessed: now });
                    break;
                case 'schema':
                    for (const sc of change.schemaChanges)
                        facts.push({ id: randomUUID(), type: 'decision', content: `Schema ${sc.type}: ${sc.entity}${sc.field ? '.' + sc.field : ''} in ${sc.file}`, confidence: 0.9, timestamp: ts, source, tags: ['schema', sc.entity], accessCount: 0, lastAccessed: now });
                    break;
                case 'api':
                    for (const ac of change.apiChanges)
                        facts.push({ id: randomUUID(), type: 'decision', content: `API ${ac.type}${ac.endpoint ? ' at ' + ac.endpoint : ''} in ${ac.file}`, confidence: 0.9, timestamp: ts, source, tags: ['api', ac.file], accessCount: 0, lastAccessed: now });
                    break;
                case 'security':
    ...
    deduplicate(facts) {
        ...
        for (const fact of facts) {
            ...
            if (!existing || fact.confidence > existing.confidence)
                seen.set(normalized, { ...fact, confidence: Math.max(fact.confidence, existing?.confidence || 0) });
        }
        return [...seen.values()].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    }
    semanticCluster(facts) {
        ...
        for (const [, clusterFacts] of clusters) {
            if (clusterFacts.length <= 3)
                merged.push(...clusterFacts);
            else {
    ...
    rankByImportance(facts) {
        const priority = { milestone: 0, decision: 1, lesson: 2, risk: 3, metric: 4, fact: 5 };
