...
export class Pipeline {
    ...
    async run() {
        ...
        const handoff = this.contextBuilder.buildHandoff('vcea', 'next', memory, ctxGraph, track);
        ...
        const benchmark = this.benchmarker.runBenchmark(memory.stats.compressionStrategy, memory.stats.originalTokens, memory.stats.compressedTokens, evaluation.overallScore, 0, { strategy: memory.stats.compressionStrategy });
        return {
            observer: events,
    ...
    stopWatch() { this.observer.stopWatching(); }
    getGraph() { return this.graph.load(); }
    getKnowledgeBrain() { return this.knowledge; }
    getResearchEngine() { return this.research; }
    getBenchmarker() { return this.benchmarker; }
    getEvaluator() { return this.evaluator; }
    getContextBuilder() { return this.contextBuilder; }
    queryGraph(query) {
    ...
    advanceSprint() { this.tracking.advanceSprint(); }
}
