...
export class ConfigManager {
    static load(repoPath, outputDir) {
    ...
    static createDefaultConfig(repoPath, outputDir) {
        return {
            repoPath,
            outputDir,
            watchMode: false,
    ...
    static validate(config) {
        ...
        if (config.retention) {
            if (config.retention.shortTermDays < 1)
                errors.push('shortTermDays must be >= 1');
    ...
    static mergeDefaults(defaults, override) {
        return {
            ...defaults,
            ...override,
