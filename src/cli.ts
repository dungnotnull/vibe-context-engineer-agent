...
import { GovernanceManager } from './core/governance.js';
import { PluginSystem } from './core/plugin-system.js';
import { ConfigManager } from './core/config-manager.js';
import type { ManagedConfig, PipelineResult, KnowledgeItem } from './core/types.js';
import * as path from 'node:path';
import * as fs from 'node:fs';

const DEFAULT_OUTPUT = '.vcea';

function createConfig(repoPath: string, options: Record<string, string>): ManagedConfig {
  ...
  return {
    ...
    watchMode: false,
    agentTypes,
    graphStorage: (options.graphStorage as 'json' | 'neo4j') || 'json',
...
function formatResult(result: PipelineResult): void {
  const box = '═'.repeat(45);
  console.log(`\n${box}`);
  ...
  for (const event of result.observer.slice(0, 5)) console.log(`   [${event.type}] ${event.summary}`);
  if (result.observer.length > 5) console.log(`   ... and ${result.observer.length - 5} more`);
  ...
  if (result.intelligence.length > 3) console.log(`   ... and ${result.intelligence.length - 3} more`);
  ...
  if (result.knowledge && result.knowledge.length > 0) {
    ...
    for (const item of result.knowledge.slice(0, 3)) console.log(`   [${item.category}] ${item.summary.slice(0, 80)}`);
  }
...
program
  ...
  .action(async (repoPath: string, options: Record<string, string>) => {
    try {
  ...
  .action(async (repoPath: string, options: Record<string, string>) => {
    ...
    if (options.cluster) {
      ...
      for (const [cid, items] of clusters) {
        ...
        for (const item of items.slice(0, 3)) console.log(`  - ${item.summary.slice(0, 80)}`);
        console.log('');
      }
      return;
    }
    ...
    for (const item of items.slice(0, 20)) {
      ...
      if (item.keyFindings.length > 0) console.log(`  Tags: ${item.keyFindings.join(', ')}`);
  ...
  .action(async (repoPath: string, options: Record<string, string>) => {
    ...
    if (options.record) {
      ...
      const signal = collector.record({
        ...
        type: (options.type || 'context-quality') as any,
...
program
  ...
  .action(async (repoPath: string, options: Record<string, string>) => {
    ...
    if (options.show) {
      ...
      for (const m of memory.members) console.log(`  - ${m.name} (${m.role})`);
