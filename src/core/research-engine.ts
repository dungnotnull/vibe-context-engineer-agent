...
import type { KnowledgeItem, KnowledgeCategory, CompressionBenchmark, QualityEvaluation } from './types.js';
...
export class ResearchEngine {
  ...
  private memoryReviews: MemorySystemReview[] = [];
  private agentStudies: AgentArchitectureStudy[] = [];
  ...
  evaluateCompressionQuality(
    ...
    const foundFacts = referenceFacts.filter((rf) =>
      compressedFacts.some((cf) => cf.toLowerCase().includes(rf.toLowerCase()))
    );
    ...
    if (relevance < 0.8) suggestions.push('Improve fact relevance filtering');
    if (completeness < 0.8) suggestions.push('Add missing reference facts');
    if (conciseness < 0.7) suggestions.push('Remove redundant or low-value facts');
  ...
  private classifyPaperCategory(paper: PaperMetadata): KnowledgeCategory {
  ...
  private computePaperRelevance(paper: PaperMetadata): number {
    let score = 0.5;
    const text = `${paper.title} ${paper.abstract}`.toLowerCase();
    if (text.includes('context')) score += 0.15;
    if (text.includes('memory') || text.includes('retrieval')) score += 0.1;
    if (text.includes('agent')) score += 0.1;
    if (text.includes('token') || text.includes('compression')) score += 0.15;
    return Math.min(1, score + paper.citations / 10000);
  }

  private saveBenchmarks(benchmarks: CompressionBenchmark[]): void {
    fs.writeFileSync(
  ...
  saveReports(): void {
    fs.writeFileSync(
      path.join(this.outputDir, 'memory-systems-review.md'),
      this.generateMemorySystemsReport()
    );
    fs.writeFileSync(
      path.join(this.outputDir, 'agent-architecture-study.md'),
      this.generateAgentArchitectureReport()
    );
  }
}
