...
import type { CompressedMemory, ContextGraph, DevelopmentTrack, AgentContextPackage, VceaConfig, AgentProfile } from '../../core/types.js';
...
export class AgentContextBuilder {
  ...
  build(memory: CompressedMemory, graph: ContextGraph, track: DevelopmentTrack): AgentContextPackage[] {
    const packages: AgentContextPackage[] = [];
  ...
  buildHandoff(from: string, to: string, memory: CompressedMemory, graph: ContextGraph, track: DevelopmentTrack) {
  ...
  computeDiff(cacheKey: string, state: { memory: CompressedMemory; graph: ContextGraph; track: DevelopmentTrack }) {
    return this.contextDiffEngine.diff(cacheKey, state);
  }

  private buildForAgent(agentType: string, profile: AgentProfile, memory: CompressedMemory, graph: ContextGraph, track: DevelopmentTrack): AgentContextPackage {
    const layers: string[] = [];
    let content: string;
  ...
  private buildMarkdown(profile: AgentProfile, memory: CompressedMemory, graph: ContextGraph, track: DevelopmentTrack, layersOut: string[]): string {
    let md = `# Context Package for ${profile.name}\n\n> Generated: ${new Date().toISOString()}\n> Token Budget: ${profile.maxTokens}\n\n`;

    layersOut.push('sprint');
    md += '## Current Sprint\n\n**In Progress:**\n';
    for (const item of track.inProgress.slice(0, 5)) md += `- ${item.content}\n`;
    md += '\n**Recent Completions:**\n';
    for (const item of track.completed.slice(0, 5)) md += `- ✓ ${item.content}\n`;
    md += '\n';

    if (track.decisions.length > 0) {
      layersOut.push('decisions');
      md += '## Recent Decisions\n\n';
      for (const d of track.decisions.slice(0, 5)) md += `- 📐 ${d.content}\n`;
      md += '\n';
    }
    ...
    if (profile.preferences.includeHistory) {
      ...
      md += '## Project Memory\n\n';
      for (const fact of memory.facts.slice(0, 20)) md += `- [${fact.type}] ${fact.content}\n`;
      md += '\n';
    }
    ...
    if (profile.preferences.includeGraph) {
      ...
      for (const n of componentNodes.slice(0, 10)) md += `- \`${n.label}\`\n`;
      const featureNodes = graph.nodes.filter((n) => n.type === 'feature');
      if (featureNodes.length > 0) {
        md += `\n**Features (${featureNodes.length}):**\n`;
        for (const n of featureNodes.slice(0, 5)) md += `- ${n.label}\n`;
      }
      const deps = graph.relations.filter((r) => r.type === 'depends_on');
      if (deps.length > 0) {
        md += '\n**Key Dependencies:**\n';
        for (const r of deps.slice(0, 10)) md += `- \`${r.source}\` → depends_on → \`${r.target}\`\n`;
      }
      md += '\n';
    }
    ...
    if (track.blockers.length > 0) {
      ...
      md += '## ⚠️ Blockers\n\n';
      for (const b of track.blockers) md += `- 🚫 ${b.content}\n`;
      md += '\n';
    }
  ...
  private buildJson(profile: AgentProfile, memory: CompressedMemory, graph: ContextGraph, track: DevelopmentTrack, layersOut: string[]): string {
    const pkg = {
      agent: profile.name,
      generated: new Date().toISOString(),
  ...
  private buildYaml(_profile: AgentProfile, memory: CompressedMemory, graph: ContextGraph, track: DevelopmentTrack, layersOut: string[]): string {
    layersOut.push('sprint', 'decisions', 'memory', 'graph');
    return [
      '# Context Package',
      `generated: ${new Date().toISOString()}`,
  ...
  private writePackage(pkg: AgentContextPackage): void {
