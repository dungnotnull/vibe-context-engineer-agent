...
export class ContextDiffEngine {
  private cache: Map<string, { hash: string; snapshot: string }> = new Map();

  diff(
    cacheKey: string,
    current: { memory: CompressedMemory; graph: ContextGraph; track: DevelopmentTrack }
  ): ContextDiff | null {
    ...
    const previous = JSON.parse(cached.snapshot) as {
      facts: { id: string; content: string; confidence: number }[];
      decisions: string[];
      blockers: string[];
    };
    const currentParsed = JSON.parse(currentSnapshot) as typeof previous;
    ...
    const modifiedFacts: { before: MemoryFact; after: MemoryFact }[] = [];
    ...
    const newDecisions = currentParsed.decisions.filter(
      (d) => !previous.decisions.includes(d)
    );
    const resolvedBlockers = previous.blockers.filter(
      (b) => !currentParsed.blockers.includes(b)
    );
