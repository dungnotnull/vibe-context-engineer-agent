...
export class HandoffEngine {
  ...
  private buildSummary(track: DevelopmentTrack, memory: CompressedMemory): string {
    const lines: string[] = [];
  ...
  private buildCurrentState(track: DevelopmentTrack, graph: ContextGraph): string {
    const lines: string[] = [];
  ...
  private buildNextSteps(track: DevelopmentTrack, graph: ContextGraph): string[] {
    const steps: string[] = [];
    ...
    const staleComponents = graph.nodes.filter(
      (n) =>
        n.type === 'component' &&
        n.metadata.lastChanged &&
        new Date(n.metadata.lastChanged as string).getTime() < Date.now() - 30 * 24 * 60 * 60 * 1000
    );
