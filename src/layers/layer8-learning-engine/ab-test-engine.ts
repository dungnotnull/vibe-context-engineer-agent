...
export class ABTestEngine {
  ...
  concludeTest(testId: string): ABTestResult | null {
    const test = this.activeTests.get(testId);
    if (!test || test.resultsA.length === 0 || test.resultsB.length === 0) return null;
...
interface ABTestConfig {
  strategyA: string;
  strategyB: string;
  resultsA: Array<{ qualityScore: number; reductionRatio: number }>;
  resultsB: Array<{ qualityScore: number; reductionRatio: number }>;
}
