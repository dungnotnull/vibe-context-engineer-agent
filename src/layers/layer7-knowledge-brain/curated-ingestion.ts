...
export class CuratedIngestionEngine {
  ...
  dequeueBatch(maxItems = 10): Array<{ id: string; content: IngestedContent }> {
    const queue = this.getQueue();
  ...
  private extractFindings(raw: string): string[] {
    const sentences = raw
      .split(/[.!?\n]+/)
      .map((s) => s.trim())
  ...
  private estimateConfidence(content: IngestedContent): number {
    let confidence = 0.5;
    if (content.sourceType === 'research-paper') confidence += 0.25;
    if (content.sourceType === 'documentation') confidence += 0.2;
    if (content.sourceType === 'benchmark') confidence += 0.3;
    if (content.raw.length > 500) confidence += 0.1;
    return Math.min(1, confidence);
  }

  private computeRelevance(content: IngestedContent): number {
  ...
  private getQueue(): Array<{ id: string; enqueuedAt: string; content: IngestedContent }> {
    try {
