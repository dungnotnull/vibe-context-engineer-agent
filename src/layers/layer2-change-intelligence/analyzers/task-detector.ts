import type { ChangeEvent, TaskCompletionResult, DetectedTask } from '../../../core/types.js';
...
export class TaskDetector {
  detect(event: ChangeEvent): TaskCompletionResult {
    ...
    const taskPatterns = [
      { pattern: /(?:fix(?:es|ed)?|resolve[sd]?)\s+(?:#\d+|bug|issue)/i, status: 'completed' as const, confidence: 0.9 },
      { pattern: /(?:implement[sd]?|add[sd]?)\s+(?:feature|support|ability)/i, status: 'completed' as const, confidence: 0.85 },
      { pattern: /(?:close[sd]?|complete[sd]?)\s+(?:#\d+|task|ticket)/i, status: 'completed' as const, confidence: 0.95 },
      { pattern: /wip|work in progress|in progress/i, status: 'partial' as const, confidence: 0.8 },
      { pattern: /todo|to-do|needs work|remaining/i, status: 'partial' as const, confidence: 0.7 },
      { pattern: /revert|rollback|undo/i, status: 'abandoned' as const, confidence: 0.75 },
      { pattern: /deprecated|removed|deleted/i, status: 'abandoned' as const, confidence: 0.7 },
    ];
