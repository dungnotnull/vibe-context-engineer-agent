import type { ClassifiedChange, HierarchicalSummary, FileDiffDetail } from '../../core/types.js';

export class HierarchicalSummarizer {
  summarize(changes: ClassifiedChange[]): HierarchicalSummary | null {
    if (changes.length === 0) return null;

    const allFileDetails: FileDiffDetail[] = [];
    ...
    if (allFileDetails.length === 0) return null;
  ...
  private buildFileLevel(details: FileDiffDetail[]): HierarchicalSummary[] {
  ...
  private buildModuleLevel(fileSummaries: HierarchicalSummary[]): HierarchicalSummary[] {
    const modules = new Map<string, HierarchicalSummary[]>();
    ...
    const result: HierarchicalSummary[] = [];
  ...
  private buildSystemLevel(
    moduleSummaries: HierarchicalSummary[],
  ...
  private extractKeyPoints(detail: FileDiffDetail): string[] {
    const points: string[] = [];
