...
export class StaleDetector {
  ...
  private checkStaleness(fact: MemoryFact, ageMs: number, allFacts: MemoryFact[]): boolean {
    // Hard TTL check
    if (fact.ttl && ageMs > fact.ttl) return true;

    // Soft staleness: low confidence + old
    if (fact.confidence < 0.5 && ageMs > 7 * 24 * 60 * 60 * 1000) return true;

    // Access-based staleness
    const daysSinceAccess = (Date.now() - new Date(fact.lastAccessed).getTime()) / (24 * 60 * 60 * 1000);
    if (fact.accessCount < 3 && daysSinceAccess > 30) return true;

    // Duplicate fact detected (same content, different IDs)
    const similar = allFacts.filter(
      (f) =>
        f.id !== fact.id &&
  ...
  private generateRecommendations(stalenessRatio: number, staleCount: number, totalCount: number): string[] {
    const recommendations: string[] = [];

    if (stalenessRatio > 0.3) {
      recommendations.push(
        `High staleness (${(stalenessRatio * 100).toFixed(0)}%): Consider running cleanup or reviewing compression strategy`
      );
    }
    if (stalenessRatio > 0.5) {
      recommendations.push(
        `Critical staleness: ${staleCount}/${totalCount} facts are stale. Run forced cleanup.`
      );
    }
