import type { ContextGraph, ImpactAnalysis, ImpactedNode, GraphPath } from '../../core/types.js';
...
export class ImpactAnalyzer {
  analyze(graph: ContextGraph, sourceNodeId: string): ImpactAnalysis {
    ...
    const queue: { id: string; distance: number; chain: string[] }[] = [
  ...
  calculateChangeRisk(graph: ContextGraph, changedNodeIds: string[]): {
    ...
    const highRiskNodes = [...allImpacted].filter((id) => {
      const centrality = graph.relations.filter(
    ...
    if (totalRisk > 50 || highRiskNodes.length > 10) riskLevel = 'critical';
    else if (totalRisk > 20 || highRiskNodes.length > 5) riskLevel = 'high';
    else if (totalRisk > 5 || highRiskNodes.length > 2) riskLevel = 'medium';
    else riskLevel = 'low';
